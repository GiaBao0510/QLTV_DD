...
class BanvangController with ChangeNotifier {
  ...
  Future<TruocKhiThucHienBanVang_model> ThongTinSanPham() async {
    ...
    final res = await http.get(
        Uri.parse(
            '$url/api/giaodich/kiemtramahanghoa?HANGHOAMA=${ThuVienUntilState.maHangHoa}'),
        headers: {
          "accesstoken": "${prefs.getString('accesstoken')}",
        });

    List<dynamic> json = jsonDecode(res.body);
    List<TruocKhiThucHienBanVang_model> thongTinHangHoa =
        json.map((e) => TruocKhiThucHienBanVang_model.fromMap(e)).toList();
  ...
  //Lấy thong tin hang hoa Plus
  // ignore: non_constant_identifier_names
  Future<TruocKhiThucHienBanVang_model> FecthThongTinSanPham(
      {String maSP = ''}) async {
    // String ma = ThuVienUntilState.maHangHoa.isEmpty
    //     ? maSP
    //     : ThuVienUntilState.maHangHoa;
    print('Ma Controller: $maSP');
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    final res = await http.get(
        Uri.parse('$url/api/giaodich/kiemtramahanghoa?HANGHOAMA=$maSP'),
        headers: {
          "accesstoken": "${prefs.getString('accesstoken')}",
        });
