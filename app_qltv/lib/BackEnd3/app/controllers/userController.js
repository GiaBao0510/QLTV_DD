...
exports.createGroupUser = async (req, res, next) => {
  try {
    const newUser = await userService.createGroupUser(req.body);
    res.status(201).json(newUser);
  } catch (error) {
    next(error);
  }
...
exports.updateGroupUser = async (req, res, next)=>{
  try{
    const update = await userService.updateGroupUser(req.params.id,req.body);
    if(!update){
      return res.status(404).json({message:'User not found'});
    }
    return res.status(200).json({message: 'User was update successfully'})
...
exports.deleteGroupUser = async (req, res, next)=>{
  try{
    const deleted = await userService.deleteGroupUser(req.params.id);
    if(!deleted){
      return res.status(404).json({message:'User not found'});
    }
    return res.status(200).json({message: 'User was delete successfully'})
...
exports.getGroupUserById = async (req, res, next) => {
  try {
    const user = await userService.getGroupUserById(req.params.id);
    res.status(200).json(user);
  } catch (error) {
    next(error);
  }
